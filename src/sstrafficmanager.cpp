...
queue<Flit* > retardq;
queue<int > retardt;
SSTrafficManager::SSTrafficManager(  const Configuration &config, const vector<Booksim_Network *> & net )
  : TrafficManager(config, net)
{
  gWatchOut = &cout;
  vc_classes=0;
  nodes.resize(_nodes);
  vc_ptrs = new int[ _nodes ];
  memset(vc_ptrs,0, _nodes *sizeof(int));
  ...
  for(int i = 0; i<Memory::CacheRequest::SignalOriginalAck+1; i++){
    type_pair_sent.push_back( new Stats(this, "type",1.0, _nodes*_nodes));
  }
  ...
  for(int c = 0; c < _classes; ++c) {
    ...
    if(_stats_out) {
      *_stats_out << "lat_post"<<left<<"(" << c+1 << ") = " << cur_latency << ";" << endl
		  << "lat_hist_post"<<left<<"(" << c+1 << ",:) = " << *_plat_stats[c] << ";" << endl
	...
	for(unsigned int j = 0; j < _nodes; ++j) {
      ...
      *_stats_out << "];" << endl
		  << "sent_post"<<left<<"(" << c+1 << ",:) = [ ";
      for ( unsigned int d = 0; d < _nodes; ++d ) {
...
void SSTrafficManager::DisplayStats(ostream & os) {
  ...
  for(int c = 0; c < _classes; ++c) {
    ...
    cout << "Class " << c+1 << ":" << endl;
    cout << "Number of packets = "<< _plat_stats[c]->NumSamples()<<endl;
...
void SSTrafficManager::_RetireFlit( Flit *f, int dest )
{
  ...
  if(f){
	...
	last_trace = _time;
      ...
      type_pair_sent[(int)mmsg->_request->_type]->AddSample(f->src*_nodes+dest);
				      ...
				      int cl, int time )
{
  ...
  for ( int i = 0; i < size; ++i ) {
    ...
    f->subnetwork = subnetwork;
    f->src    = source;
    //find the actual time the message is generated for notification
    f->time   = msg->SourceTime.value();
    f->ttime  = ttime;
    f->record = record;
    f->cl     = cl;
    if(i == size-1){
  ...
  for(int source = 0; source < _nodes; ++source) {
	...
	if(!_partial_packets[source][c].empty()) {
	  ...
	  if(f->head && f->vc == -1) { // Find first available VC
	    ...
	    assert(_time>=f->time);
	    if(_pri_type == forward_note  && (_time-f->time > NOTIFICATION_TIME_THRESHOLD || NOTIFICATION_TIME_THRESHOLD==0)){
